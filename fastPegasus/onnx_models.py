...
class PegasusEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        encoder_hidden_state = torch.from_numpy(
            ...
            )[0] # idx==0, taking the hidden_states with the shape of (batch,src_seq_len,d_model)
        )
...
class PegasusDecoderInit(torch.nn.Module):
    ...
    def forward(self, input_ids, encoder_attention_mask, encoder_hidden_states):
        ...
        decoder_outputs = self.decoder.run( # decoderInit does not have past_key_values for inputs
            None,
            {
                "input_ids": input_ids.cpu().numpy(),
...
class PegasusDecoder(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask, encoder_output, past_key_values):
        ...
        decoder_outputs = self.decoder.run(None, {**decoder_inputs, **past_key_values}) # past_key_value
...
class OnnxPegasus(PegasusForConditionalGeneration):
    ...
    def get_encoder(self): # neccessary
        return self.encoder

    def get_decoder(self): # neccessary
        return self.decoder

    def get_output_embeddings(self): # neccessary
        return None
    ...
    def forward(
        ...
        # this function stimulates the process of forward function of class PegasusForConditionalGenearation
        # encode
        ...
        if past_key_values is not None:
            # if past_key_values is available，only the last token of decoder_input_ids is needed (speed up for decode).
...
def export_and_get_onnx_model(
    ...
    # Step 1. convert huggingface pegasus model to onnx
...
def get_onnx_model(model_name, onnx_models_path=saved_models_path, quantized=True):
    ...
    model = OnnxPegasus(model_name, model_sessions)
